...
export class RegistrationFluxStepService extends AbstractService {
  ...
  get(data): Observable<Response<any>> {
    return this.http.get<Response<any>>(`${this.coreUrl}/${this.routeUrl}`, { params: data });
  }

  add(data: RegistrationFluxStep): Observable<Response<number>> {
    return this.http.post<Response<number>>(`${this.coreUrl}/${this.routeUrl}`, data);
  }

  update(data): Observable<Response<any>> {
    return this.http.patch<Response<any>>(`${this.coreUrl}/${this.routeUrl}`, data);
  }
}
