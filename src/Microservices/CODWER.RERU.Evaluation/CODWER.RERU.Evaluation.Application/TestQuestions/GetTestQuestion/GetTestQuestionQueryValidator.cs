...
namespace CODWER.RERU.Evaluation.Application.TestQuestions.GetTestQuestion
{
    public class GetTestQuestionQueryValidator : AbstractValidator<GetTestQuestionQuery>
    {
        ...
        public GetTestQuestionQueryValidator(AppDbContext appDbContext)
        {
            ...
            When(x => appDbContext.Tests.Include(x => x.TestType).ThenInclude(x => x.Settings).FirstOrDefault(t => t.Id == x.TestId).TestType.Settings.MaxErrors.HasValue, () =>
            {
                RuleFor(x => x)
            ...
            When(x => !appDbContext.Tests.Include(x => x.TestType).ThenInclude(x => x.Settings).FirstOrDefault(t => t.Id == x.TestId).TestType.Settings.PossibleChangeAnswer, () =>
            {
                RuleFor(x => x)
            ...
            When(x => !appDbContext.Tests.Include(x => x.TestType).ThenInclude(x => x.Settings).First(t => t.Id == x.TestId).TestType.Settings.PossibleGetToSkipped, () =>
            {
                RuleFor(x => x)
            ...
            When(x => appDbContext.Tests.Include(x => x.TestType).First(t => t.Id == x.TestId).TestType.Mode == TestTypeModeEnum.Test, () =>
            {
                RuleFor(x => x)
