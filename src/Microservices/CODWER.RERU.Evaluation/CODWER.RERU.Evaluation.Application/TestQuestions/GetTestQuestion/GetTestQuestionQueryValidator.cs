...
namespace CODWER.RERU.Evaluation.Application.TestQuestions.GetTestQuestion
{
    public class GetTestQuestionQueryValidator : AbstractValidator<GetTestQuestionQuery>
    {
        ...
        public GetTestQuestionQueryValidator(AppDbContext appDbContext)
        {
            ...
            When(x => appDbContext.Tests.Include(x => x.TestType).ThenInclude(x => x.Settings).FirstOrDefault(t => t.Id == x.Data.TestId).TestType.Settings.MaxErrors.HasValue, () =>
            {
                RuleFor(x => x.Data)
            ...
            When(x => appDbContext.Tests.Include(x => x.TestType).First(t => t.Id == x.Data.TestId).TestType.Mode == TestTypeModeEnum.Test, () =>
            {
                RuleFor(x => x.Data)
