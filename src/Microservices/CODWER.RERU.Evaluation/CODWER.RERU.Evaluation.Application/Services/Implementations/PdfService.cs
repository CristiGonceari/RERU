...
namespace CODWER.RERU.Evaluation.Application.Services.Implementations
{
    public class PdfService : IPdfService
    {
        ...
        public async Task<FileDataDto> PrintTestPdf(int testId)
        {
            ...
            return await GetPdf(item, false);
        }

        public async Task<FileDataDto> PrintTestResultPdf(int testId)
        {
            var item = _appDbContext.Tests
        ...
        private async Task<FileDataDto> GetPdf(Test item, bool viewResult)
        {
            string path = null;
            string source;
            ...
            if (viewResult)
            {
                path = new FileInfo("PdfTemplates/TestResult.html").FullName;
                myDictionary = await GetDictionary(item, true);
            }
            else
            {
                path = new FileInfo("PdfTemplates/Test.html").FullName;
                myDictionary = await GetDictionary(item, false);
            }
            ...
            if (viewResult)
            {
                return FileDataDto.GetPdf("Test_Result.pdf", res);
            }
        ...
        private async Task<FileDataDto> GetPdf(QuestionUnit items, FileDataDto getQuestionFile)
        {
            ...
            return FileDataDto.GetPdf("Intrebarea.pdf", res);
        }
        ...
        private async Task<Dictionary<string, string>> GetDictionary(Test item, bool viewResult)
        {
            var myDictionary = new Dictionary<string, string>();
            ...
            if (viewResult)
            {
                ...
                myDictionary.Add("{duration}", item.TestTemplate.Duration.ToString());
                myDictionary.Add("{evaluator_name}", GetEvaluatorName(item));
                myDictionary.Add("{programmed_time}", item.ProgrammedTime.ToString("dd-MM-yyyy"));
                myDictionary.Add("{acumulated_points}", item.TestQuestions.Select(x => x.Points).Sum().ToString());
        ...
        private async Task<string> GetTableContent(Test item, bool view)
        {
        ...
        private async Task<string> GetTestQuestionResultContent(Test item)
        {
            var content = string.Empty;

            foreach (var testQuestion in item.TestQuestions.Select((value, i) => new { i, value }))
            {
                content += $@"<div style=""margin-bottom: 20px; width: 930px;""><b>{testQuestion.i + 1}.</b></div>";
                content = await GetQuestionResultTemplateByType(testQuestion.value, content);
            }
        ...
        private async Task<string> GetQuestionResultTemplateByType(TestQuestion testQuestion, string content)
        {
            ...
            {
                content = await GetFreeTextQuestion(testQuestion, content);
            }

            if (testQuestion.QuestionUnit.QuestionType == QuestionTypeEnum.MultipleAnswers || testQuestion.QuestionUnit.QuestionType == QuestionTypeEnum.OneAnswer)
            {
                content = await GetOneAndMultipleAnswersQuestion(testQuestion, content);
            }
        ...
        private async Task<string> GetFooterContent(Test item)
        {
            ...
            content += $@"<div style=""margin-top: 50px;"">
                            <h2 style=""text-align: left; font-size: 22px;"">Evaluator: {GetEvaluatorName(item)}</h2>
        ...
        private async Task<string> GetHashedAnswerQuestion(TestQuestion testQuestion, string content)
        {
            ...
            if (question.Options != null)
            {
                foreach (var option in question.Options)
                {
                    question.Question = question.Question.Replace($"[answer]{option.Answer}[/answer]", $@"<input type=""text"">");
                }
            }
        ...
        private async Task<string> GetOneAndMultipleAnswersQuestion(TestQuestion testQuestion, string content)
        {
            content +=$@"<table style=""border: 1px solid black; border-collapse: collapse; width: 100%; margin-bottom: 20px; vertical-align: middle; font-weight: 100"">
                            <tr>
                                <th style=""border: 1px solid black; border-collapse: collapse; text-align: left; padding-left: 5px; height: 30px; margin: 5px 5px 5px 5px;"" colspan=""2"">
                                    <span style=""text-decoration: underline; margin: 5px 5px 5px 5px;"">Întrebarea:</span> <br> 
                                    {testQuestion.QuestionUnit.Question}
