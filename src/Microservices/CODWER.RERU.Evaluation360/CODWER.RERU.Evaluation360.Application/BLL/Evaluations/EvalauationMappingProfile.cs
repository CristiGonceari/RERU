    ...
    public class EvalauationMappingProfile: Profile
    {
        public EvalauationMappingProfile()
        {
            ...
            CreateMap<Evaluation, GetEvaluationDto>()
                ...
                .ForMember(dest=> dest.EvaluatedName, opts=> opts.MapFrom(src=> $"{src.EvaluatedUserProfile.FullName}" ))
                .ForMember(dest=> dest.CounterSignerName, opts=> opts.MapFrom(src=> $"{src.CounterSignerUserProfile.FullName}" ))
                .ForMember(dest=> dest.FunctionSubdivision, opts=> opts.MapFrom(src=> $"{src.EvaluatedUserProfile.EmployeeFunctions.Name}"))
                .ForMember(dest=> dest.FunctionEvaluator, opts=> opts.MapFrom(src=> $"{src.EvaluatorUserProfile.EmployeeFunctions.Name}"))
                .ForMember(dest=> dest.FunctionCounterSigner, opts=> opts.MapFrom(src=> $"{src.CounterSignerUserProfile.EmployeeFunctions.Name}"))
                ;
