...
using CVU.ERP.Module.Common.Exceptions;
...
namespace CVU.ERP.Module.API.Middlewares.ResponseWrapper {
    public class ResponseWrapperMiddleware {
        ...
        public virtual async Task InvokeAsync (HttpContext context) {
            if (IsSwaggerRequest (context, _options.SwaggerPath) || !IsApiRequest (context))
                ...
                try {
                    ...
                    isRequestOk = IsRequestSuccessful (context.Response.StatusCode);
                    if (isRequestOk) {
        ...
        public async Task HandleExceptionAsync (HttpContext context, System.Exception exception) {
            ...
            var response = new Response ();
            if (exception is ApplicationRequestValidationException) {
                var validationException = exception as ApplicationRequestValidationException;
                response.AddErrorMessages (validationException.ValidationMessages.Select (vm => new ErrorMessage (vm)));
